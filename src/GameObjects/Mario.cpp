...
void Mario::shoot() {
    ...
    if (this->getComponent<StateMachine>()->getIsLeft()) {
        current_scene->addObject(std::make_shared<FireBall>(this, position.x - 32.f, position.y, -600.f));
    } else {
        current_scene->addObject(std::make_shared<FireBall>(this, position.x +
            getComponent<Collision>()->getOffset().x + this->getSize().x, position.y, 600.f));
    }
}
